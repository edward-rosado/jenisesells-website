...
public class AgentNotifierTests
{
    private readonly Mock<IWhatsAppSender> _whatsAppSender = new();
    private readonly Mock<IGmailSender> _gmailSender = new();
    private readonly Mock<ILogger<AgentNotifier>> _logger = new();
    private readonly AgentNotifier _notifier;

    private static readonly CancellationToken Ct = CancellationToken.None;

    public AgentNotifierTests()
    {
        _notifier = new AgentNotifier(_whatsAppSender.Object, _gmailSender.Object, _logger.Object);
    }
    ...
    {
        var lead = MakeLead();
        var score = MakeScore(75); // Bucket = "Hot"
        var config = MakeConfig(whatsAppPhoneNumberId: null); // force email path so we can inspect
        ...
        // Use WhatsApp config for this test
    ...
    public async Task NotifyAsync_Email_SellerDetails_ContainsAddressEstimatedValueCompCount()
    {
        ...
        var config = MakeConfig(whatsAppPhoneNumberId: null); // email path
    ...
    // ─── Test 12: Fallback email uses HTML formatting with agent branding ─────
