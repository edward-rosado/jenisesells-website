...
vi.mock("next/script", () => ({
  ...
  default: ({ id, src, children }: { id?: string; src?: string; children?: string }) => (
    <script data-testid={id} data-src={src} data-content={children} />
  ),
}));
...
describe("Analytics", () => {
  ...
  it("renders GA4 scripts when google_analytics_id is provided without GTM", () => {
    ...
    expect(config.getAttribute("data-content")).toContain("G-XYZ789");
  });
  ...
  it("renders Meta Pixel script when meta_pixel_id is provided", () => {
    ...
    expect(script.getAttribute("data-content")).toContain("123456789");
  });
