...
const AGENT: AgentConfig = {
  ...
  identity: {
    ...
    phone: "(252) 555-0133",
    email: "maya@testrealty.com",
    headshot_url: "/agents/test-coastal/headshot.jpg",
  },
  location: { state: "NC" },
  branding: {
...
describe("AboutCoastal", () => {
  it("renders the section heading using data.title", () => {
    render(<AboutCoastal agent={AGENT} data={ABOUT_DATA} />);
  ...
  it("falls back to 'About {name}' when data.title is absent", () => {
    ...
    render(<AboutCoastal agent={AGENT} data={dataNoTitle} />);
    expect(screen.getByRole("heading", { level: 2 }).textContent).toMatch(/About Maya Torres/);
  });
  ...
  it("renders agent headshot when headshot_url is provided", () => {
    ...
    expect(img).toHaveAttribute("alt", "Photo of Maya Torres");
  });

  it("does not render agent photo when headshot_url is absent", () => {
    const agentNoPhoto: AgentConfig = {
      ...AGENT,
      identity: { ...AGENT.identity, headshot_url: undefined },
    };
    render(<AboutCoastal agent={agentNoPhoto} data={ABOUT_DATA} />);
    expect(screen.queryByRole("img")).not.toBeInTheDocument();
  });

  it("agent photo has teal border", () => {
    const { container } = render(<AboutCoastal agent={AGENT} data={ABOUT_DATA} />);
    // Photo wrapper should have teal border
  ...
  it("renders bio as array of paragraphs when bio is an array", () => {
    ...
    render(<AboutCoastal agent={AGENT} data={dataArrayBio} />);
