...
describe("AboutSplit", () => {
  it("renders the heading with agent name", () => {
    render(<AboutSplit agent={AGENT} data={DATA_WITH_CREDENTIALS} />);
  ...
  it("does not render credential badges when credentials is empty", () => {
    render(<AboutSplit agent={AGENT} data={DATA_NO_CREDENTIALS} />);
  ...
  it("does not render credential badges when credentials is undefined", () => {
    render(<AboutSplit agent={AGENT} data={DATA_CREDENTIALS_UNDEFINED} />);
    expect(screen.queryByRole("list", { name: "Credentials" })).not.toBeInTheDocument();
  });

  it("uses minimal agent name in heading", () => {
    render(<AboutSplit agent={AGENT_MINIMAL} data={DATA_NO_CREDENTIALS} />);
  ...
  it("renders a single credential correctly", () => {
    render(<AboutSplit agent={AGENT} data={{ bio: "Bio here", credentials: ["REALTOR"] }} />);
  ...
  it("renders multiple paragraphs when bio is an array", () => {
    ...
    render(<AboutSplit agent={AGENT} data={arrayBioData} />);
  ...
  it("renders headshot image when agent has headshot_url", () => {
    ...
    render(<AboutSplit agent={agentWithHeadshot} data={DATA_WITH_CREDENTIALS} />);
