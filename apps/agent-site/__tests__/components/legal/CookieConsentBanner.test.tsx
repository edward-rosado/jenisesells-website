...
describe("CookieConsentBanner", () => {
  ...
  it("renders the banner when no consent stored", () => {
    render(<CookieConsentBanner accountId="test-agent" />);
  ...
  it("uses agent-specific localStorage key", () => {
    render(<CookieConsentBanner accountId="agent-abc" />);
    fireEvent.click(screen.getByRole("button", { name: "Accept cookies" }));
  ...
  it("links to /privacy with accountId", () => {
    render(<CookieConsentBanner accountId="test-agent" />);
    const link = screen.getByRole("link", { name: /privacy/i });
    expect(link).toHaveAttribute("href", "/privacy?accountId=test-agent");
  });
  ...
  it("server snapshot returns pending (SSR path)", () => {
    const html = renderToString(<CookieConsentBanner accountId="test-agent" />);
    expect(html).toBe("");
  });
});
