...
import { CoastalLiving } from "@/features/templates/coastal-living";
...
describe("CoastalLiving template", () => {
  ...
  it("uses agent prop identity when provided", () => {
    render(<CoastalLiving account={ACCOUNT} content={CONTENT} agent={AGENT_PROP} />);
    expect(screen.getByRole("heading", { name: /About Explicit Agent/ })).toBeInTheDocument();
  });

  it("falls back to broker name when no agent", () => {
    render(<CoastalLiving account={ACCOUNT_BROKER_ONLY} content={CONTENT} />);
  ...
  it("falls back to brokerage name when no agent or broker", () => {
    render(<CoastalLiving account={ACCOUNT_BROKERAGE_ONLY} content={CONTENT} />);
