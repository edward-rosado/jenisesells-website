...
import { UrbanLoft } from "@/features/templates/urban-loft";
...
describe("UrbanLoft template", () => {
  ...
  it("uses agent prop identity when provided", () => {
    render(<UrbanLoft account={ACCOUNT} content={CONTENT} agent={AGENT_PROP} />);
    expect(screen.getByRole("heading", { name: /About Explicit Agent/ })).toBeInTheDocument();
  });

  it("falls back to broker name when no agent", () => {
    render(<UrbanLoft account={ACCOUNT_BROKER_ONLY} content={CONTENT} />);
  ...
  it("falls back to brokerage name when no agent or broker", () => {
    render(<UrbanLoft account={ACCOUNT_BROKERAGE_ONLY} content={CONTENT} />);
