...
import { ModernMinimal } from "@/features/templates/modern-minimal";
...
describe("ModernMinimal template", () => {
  it("always renders the Nav", () => {
    render(<ModernMinimal account={ACCOUNT} content={CONTENT} />);
  ...
  it("does not render Hero when disabled", () => {
    render(<ModernMinimal account={ACCOUNT} content={CONTENT_ALL_DISABLED} />);
  ...
  it("uses agent prop identity when provided", () => {
    render(<ModernMinimal account={ACCOUNT} content={CONTENT} agent={AGENT_PROP} />);
    expect(screen.getByRole("heading", { name: /About Explicit Agent/ })).toBeInTheDocument();
  });

  it("falls back to broker name when no agent", () => {
    render(<ModernMinimal account={ACCOUNT_BROKER_ONLY} content={CONTENT} />);
  ...
  it("falls back to brokerage name when no agent or broker", () => {
    render(<ModernMinimal account={ACCOUNT_BROKERAGE_ONLY} content={CONTENT} />);
