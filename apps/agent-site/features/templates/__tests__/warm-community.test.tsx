...
import { WarmCommunity } from "@/features/templates/warm-community";
...
describe("WarmCommunity template", () => {
  it("always renders the Nav", () => {
    render(<WarmCommunity account={ACCOUNT} content={CONTENT} />);
  ...
  it("does not render Hero when disabled", () => {
    render(<WarmCommunity account={ACCOUNT} content={CONTENT_ALL_DISABLED} />);
  ...
  it("uses agent prop identity when provided", () => {
    render(<WarmCommunity account={ACCOUNT} content={CONTENT} agent={AGENT_PROP} />);
    expect(screen.getByRole("heading", { name: /About Explicit Agent/ })).toBeInTheDocument();
  });

  it("falls back to broker name when no agent", () => {
    render(<WarmCommunity account={ACCOUNT_BROKER_ONLY} content={CONTENT} />);
  ...
  it("falls back to brokerage name when no agent or broker", () => {
    render(<WarmCommunity account={ACCOUNT_BROKERAGE_ONLY} content={CONTENT} />);
