    ...
    >
      <div style={{
        ...
        {data.credentials && data.credentials.length > 0 && (
          <ul aria-label="Credentials" style={{
            display: "flex",
            flexWrap: "wrap",
        ...
        <div style={{
          ...
          {phone && <div><a href={safeTelHref(phone)} aria-label={`Call ${displayName}`} style={{ color: "inherit", textDecoration: "none" }}>{phone}</a></div>}
          {email && <div><a href={safeMailtoHref(email)} aria-label={`Email ${displayName}`} style={{ color: "inherit", textDecoration: "none" }}>{email}</a></div>}
        </div>
      </div>
    </section>
  );
