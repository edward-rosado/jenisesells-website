...
export function Footer({ agent, accountId, locale }: FooterProps) {
  ...
  return (
    ...
    >
      ...
      >
        {title || strings.licensedRealEstateSalesperson}{licenseNumber && ` | License #${licenseNumber}`}
      </p>
      {phone && (
      ...
      >
        The information on this website is for general informational purposes only. {name}, {title || strings.licensedRealEstateSalesperson}{licenseNumber && ` (License #${licenseNumber})`}{brokerageName && `, is affiliated with ${brokerageName}`}{officeAddress && `, ${officeAddress}`}.{officePhone && ` ${officePhone}.`} All information deemed reliable but not guaranteed.
      </p>
      <nav
      ...
      >
        <a href={`/privacy${qs}`} style={{ color: "rgba(255,255,255,0.85)", textDecoration: "underline" }}>{strings.privacyPolicy}</a>
        <a href={`/terms${qs}`} style={{ color: "rgba(255,255,255,0.85)", textDecoration: "underline" }}>{strings.termsOfUse}</a>
        <a href={`/accessibility${qs}`} style={{ color: "rgba(255,255,255,0.85)", textDecoration: "underline" }}>{strings.accessibility}</a>
      </nav>
      <p
        style={{
          marginTop: "8px",
      ...
      >
        &copy; {new Date().getFullYear()} {name}. {strings.allRightsReserved}
      </p>
    </footer>
  );
}
