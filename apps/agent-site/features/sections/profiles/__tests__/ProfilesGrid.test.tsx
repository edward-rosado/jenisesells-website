...
import { ProfilesGrid } from "@/features/sections/profiles/ProfilesGrid";
...
describe("ProfilesGrid", () => {
  ...
  it("renders title and subtitle when provided", () => {
    render(<ProfilesGrid items={ITEMS} title="Meet the Team" subtitle="Our experts" />);
  ...
  it("uses id=profiles for anchor linking", () => {
    const { container } = render(<ProfilesGrid items={ITEMS} />);
  ...
  it("renders empty section when items is empty", () => {
    const { container } = render(<ProfilesGrid items={[]} />);
    expect(container.querySelector("#profiles")).toBeInTheDocument();
