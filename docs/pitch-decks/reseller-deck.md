# Real Estate Star — Reseller Recruitment Deck

---

## Slide 1: Cover

# Real Estate Star

### Sell the Future of Real Estate Tech

A reseller opportunity in real estate's most underserved market.

---

## Slide 2: The Opportunity

### 1.2 million agents. Most priced out of good tech.

- **$12.3 billion** real estate SaaS market, growing **12.9% annually**
- **54% of agents** spend $50–$500/mo on tech — but get fragmented, overpriced tools
- **Median new agent income: $8,100/yr** — they can't afford $300–$500/mo platforms
- **68% already use AI** — the appetite is there, affordable supply isn't
- **LionDesk shut down** (Sept 2025) — agents need alternatives now

> **Speaker notes:** The real estate SaaS market hits $38.3B by 2033. Solo agents are the most underserved segment — enterprise platforms ignore them, budget tools give them half-baked solutions. This is the gap.

---

## Slide 3: The Gap in the Market

### No platform combines website + CMA + lead automation + compliance under $100/mo.

**Premium tier ($300–$1,700/mo):**
KVCore, Ylopo, Chime, BoomTown — built for teams and brokerages, not solo agents.

**Budget tier ($40–$150/mo):**
Placester, iNCOM — basic websites, no CMA, no automation.

**CMA-only tools ($40–$99/mo):**
Cloud CMA, Saleswise, PropStream — separate subscriptions on top of everything else.

**Real Estate Star: $14.99/mo — everything included.**

> **Speaker notes:** Draw the market map. Point to the empty space at $15/mo with full features. Nobody is there. That's where we sit.

---

## Slide 4: Meet Real Estate Star

### Everything an agent needs. One platform. $14.99/mo.

- **Professional white-label website** — 12 beautiful, luxurious templates to choose from
- **AI-powered CMA automation** — lead comes in, branded PDF goes out
- **Lead capture** with bot protection and auto-enrichment
- **State-specific legal compliance** — disclosures, consent tracking built in
- **Google Workspace integration** — agent's own Drive, Gmail, Sheets
- **SEO & AEO ready** — found by Google search and AI answer engines
- **Google Analytics** — track visitors and leads out of the box
- **10-minute AI-powered setup** — connect Google, AI discovers their brand

> **Speaker notes:** This isn't a website builder. It's a business automation platform. The website is just the visible layer — underneath it is CMA automation, lead enrichment, compliance, and AI that learns the agent's voice and brand.

---

## Slide 5: Why It Sells Itself

### You're selling a $15/month product that replaces $300–$500/month of tools.

| What Agents Pay Now | Real Estate Star |
|---|---|
| Website: $59–$329/mo | Included |
| CMA tool: $40–$99/mo | Included |
| Lead capture: $50–$100/mo | Included |
| Compliance: manual / risky | Included |
| **Total: $149–$528+/mo** | **$14.99/mo** |

**This isn't a hard sell. It's a math problem.**

> **Speaker notes:** When agents see this table, the conversation is basically over. You're saving them $150–$500/month. The objection handling is built into the price.

---

## Slide 6: Your Economics

### You keep 100% of the setup fee. Every sale.

| Deal Type | Your Setup Fee | Agent's Monthly | Your Earnings |
|----------|---------------|----------------|--------------|
| Solo agent | $49–$149 | $14.99/mo | $49–$149 per sale |
| Small brokerage (10 agents) | $79–$99/agent | $14.99/mo/agent | $790–$990 per deal |
| Mid brokerage (25 agents) | $49–$79/agent | $14.99/mo/agent | $1,225–$1,975 per deal |
| Large brokerage (50+ agents) | Negotiated | $14.99/mo/agent | $2,450+ per deal |

**Earnings scenarios:**
- 10 solo agents/month = **$490–$1,490/mo** in setup fees
- 1 brokerage deal/month (25 agents) = **$1,225–$1,975** per deal
- No cap on earnings. No territory restrictions.

> **Speaker notes:** Compare these setup fees to competitors: KVCore charges $500–$1,000 for setup, Luxury Presence charges $500–$6,000, AgentFire charges $700–$3,500. You're charging a fraction and the agent gets MORE.

---

## Slide 7: Competitive Landscape

### How Real Estate Star compares to every major platform

| Platform | Monthly | Setup Fee | Website | CMA | Lead Auto | Compliance | AI-Native |
|----------|---------|-----------|---------|-----|-----------|------------|-----------|
| **Real Estate Star** | **$14.99** | **$0** | **Yes** | **Yes** | **Yes** | **Yes** | **Yes** |
| KVCore / BoldTrail | ~$499 | $500–$1,000 | Yes | Add-on | Yes | No | Partial |
| Ylopo | Custom | Unknown | Yes | No | Yes | No | Yes |
| Chime / Lofty | ~$449 | Unknown | Yes | No | Yes | No | Partial |
| Real Geeks | $249–$399 | $250 | Yes | No | Partial | No | No |
| Placester | $59–$329 | $0 | Yes | No | No | No | No |
| Luxury Presence | $300–$1,500 | $500–$6,000 | Yes | No | No | No | Partial |
| AgentFire | One-time | $700–$3,500 | Yes | No | No | No | No |

**Your pitch to agents is simple:**
*"You're paying $300–$500/month. This does the same thing for $15. And I'll set it up for you."*

> **Speaker notes:** Walk through this table row by row. Point out the "No" columns for competitors. Real Estate Star is the ONLY all-Yes row in the industry.

---

## Slide 8: The Product

### What agents see when they sign up

**Professional Website**
- 12 luxurious templates — modern, mobile-ready, branded to the agent
- Existing site or custom look? We accommodate (additional time)

**CMA Reports**
- AI-generated, agent-branded PDF reports
- Sent automatically when a lead submits their property

**Lead Capture**
- Forms with Turnstile bot protection
- AI enrichment and instant notifications

**Activation Flow**
- Connect Google → AI discovers brand → site goes live

> **Speaker notes:** If possible, open a live demo of jenise-buckalew.real-estate-star.com to show a real agent site.

---

## Slide 9: How Activation Works

### Agent connects Google. AI does the rest.

```
1. Agent clicks your setup link
2. Google OAuth — connects Gmail, Drive
3. AI scans their existing data (emails, contacts, listings)
4. AI extracts: brand colors, voice, personality, headshot
5. Website goes live at {handle}.real-estate-star.com
6. CMA pipeline activated — ready for leads
7. Total time: ~10 minutes
```

**You don't need to build anything. You don't need to configure anything.**
The AI does the setup.

> **Speaker notes:** This is the key differentiator for your sales process. You're not asking the agent to fill out forms or wait weeks. You send them a link, they click it, and in 10 minutes they have a professional website with CMA automation. No other platform can say that.

---

## Slide 10: Multi-Language — Your Pitch Differentiator

### Bilingual agents are an untapped market.

- **46 million Spanish speakers** in the US — and growing
- Most real estate platforms are **English-only**
- Real Estate Star supports **English + Spanish** out of the box
- Agent websites **auto-detect visitor language** and switch content
- Lead emails, CMA reports, and notifications sent in the lead's language
- AI extracts the agent's **real Spanish voice** — not machine translation

**Walk into any brokerage in a bilingual market and ask: does your tech speak Spanish?**
The answer is almost always no. Until now.

This is your **pitch differentiator** for bilingual brokerages. No competitor offers this.

> **Speaker notes:** This is your door-opener for Florida, Texas, California, New Jersey, and any market with a significant Hispanic population. Bilingual brokerages will immediately see the value — their agents can serve Spanish-speaking clients with a professional, localized experience. No other platform at any price point offers this.

---

## Slide 11: What Agents Get

### Feature breakdown — what they'd pay separately elsewhere

| Feature | Description |
|---------|-------------|
| Professional Website | 12 luxurious templates, white-label, branded, mobile-ready. Custom looks available. |
| SEO & AEO | Optimized for Google search AND AI answer engines — agents get found |
| Google Analytics | Built-in visitor and lead tracking |
| CMA Automation | Lead submits form → AI generates branded CMA PDF → emailed automatically |
| Lead Capture | Website forms with bot protection, auto-enrichment, instant notifications |
| Legal Compliance | State-specific disclosures, consent tracking, GDPR/CCPA/CAN-SPAM ready |
| Google Integration | Everything lives in the agent's Google Drive — they own their data |
| AI Brand Discovery | AI reads their emails and web presence to extract their unique voice and brand |

> **Speaker notes:** Emphasize that agents OWN their data — it lives in their Google Drive, not locked in our platform. This is a huge trust factor.

---

## Slide 12: Brokerage Deals

### Volume pricing for brokerages — your biggest earning opportunity.

- Brokerages want **uniform tech** across their agents
- One conversation → **10–50+ agent signups**
- You negotiate the setup fee per agent (suggested: $49–$99 for volume)
- Monthly stays $14.99/agent — no brokerage markup from us
- **50+ agent deals**: recurring revenue share is negotiable

**Position it as:**
*"Give your agents professional websites and automation for less than their monthly coffee budget."*

> **Speaker notes:** Brokerage deals are where the real money is. One meeting with a broker can net you $1,000–$5,000 in setup fees. And the broker looks like a hero for giving their agents professional tech at no cost to the brokerage.

---

## Slide 13: Support & Your Commitment

### What we provide to resellers:

- **Agent Sales Deck** — ready to present (you have it)
- **Setup link generation** — you send the link, agent clicks, AI does the rest
- **Product training materials** — know the product inside and out
- **Dedicated reseller support channel** — we're here when you need us
- **Feature roadmap visibility** — auto-replies, contract drafting, MLS coming soon

### Your commitment to the agent:

- **Customer satisfaction** — if the agent isn't happy with their website, you work with us to make it right
- 12 templates cover most agents, but custom looks are available (may take additional time)
- **Your reputation is tied to the product** — we make sure you look good

> **Speaker notes:** This is a partnership, not a drop-ship. The setup fee you charge is for your personal attention — making sure the agent is happy, answering their questions, being their point of contact. We handle the tech, you handle the relationship.

---

## Slide 13: Get Started

### Ready to start selling?

- **No upfront cost** to become a reseller
- **Start earning on your first sale**
- Setup fee: $49–$149 per agent (you keep 100%)
- Brokerage deals: volume pricing negotiable

**Contact:**
Eddie Rosado, Founder
Real Estate Star
https://real-estate-star.com

> **Speaker notes:** Emphasize that there's zero risk — no inventory, no upfront cost, no territory restriction. They start earning the moment they close their first agent.
