...
<#
.SYNOPSIS
    Build and deploy the Real Estate Star platform (Next.js) to Cloudflare Pages.
...
.EXAMPLE
    .\deploy-platform.ps1 -Production
    .\deploy-platform.ps1
    .\deploy-platform.ps1 -SkipBuild -Production
#>
...
# --- Configuration ---
$ProjectName   = "real-estate-star-platform"
$AccountId     = "7674efd9381763796f39ea67fe5e0505"
$CustomDomain  = "platform.real-estate-star.com"
...
Write-Host " Real Estate Star Platform -- Deploy" -ForegroundColor White
Write-Host "========================================" -ForegroundColor White
...
if (-not (Test-Path (Join-Path $PlatformDir "package.json"))) {
    Write-Fail "Platform app not found at $PlatformDir"
    exit 1
}
Write-Ok "Platform app found at $PlatformDir"

# --- Ensure open-next.config.ts exists ---
$openNextConfig = Join-Path $PlatformDir "open-next.config.ts"
...
$wranglerConfig = Join-Path $PlatformDir "wrangler.jsonc"
$wranglerToml = Join-Path $PlatformDir "wrangler.toml"
...
if (-not $SkipBuild) {
    ...
    $envFile = Join-Path $PlatformDir ".env.production"
    Set-Content -Path $envFile -Value "NEXT_PUBLIC_API_URL=$ApiUrl" -Encoding ASCII
    Write-Ok "Created .env.production (NEXT_PUBLIC_API_URL=$ApiUrl)"

    # Temporarily rename .env.local so it does not override .env.production during build
    $envLocal = Join-Path $PlatformDir ".env.local"
    $envLocalBak = Join-Path $PlatformDir ".env.local.bak"
    $movedEnvLocal = $false
    ...
    $openNextDir = Join-Path $PlatformDir ".open-next"
    ...
    $dockerPlatformDir = $PlatformDir -replace '\\', '/'
    # Convert C:\... to /c/... for Docker mount
    if ($dockerPlatformDir -match '^([A-Za-z]):(.*)') {
        $driveLetter = $Matches[1].ToLower()
        $restOfPath = $Matches[2]
        $dockerPlatformDir = "/$driveLetter$restOfPath"
    }

    Write-Info "Mounting $dockerPlatformDir into container..."
    Write-Info "Running: npm install && npx opennextjs-cloudflare build"
    ...
    docker run --rm `
        -v "${dockerPlatformDir}:/app" `
        -w /app `
        -e "NEXT_PUBLIC_API_URL=$ApiUrl" `
        $DockerImage `
        sh -c "npm install && npx opennextjs-cloudflare build" 2>&1 | ForEach-Object { Write-Host "    $_" }
...
# --- Verify build output ---
$workerJs = Join-Path $PlatformDir ".open-next\worker.js"
...
$hasWrangler = Test-Path (Join-Path $PlatformDir "node_modules\.bin\wrangler.cmd")
if (-not $hasWrangler) {
...
if ($Production) {
    ...
    Write-Host "    3. Enter: $CustomDomain" -ForegroundColor Yellow
    Write-Host "    4. Also add: www.real-estate-star.com" -ForegroundColor Yellow
} else {
    Write-Info "Preview URL will be shown in the wrangler output above"
    Write-Info "Deploy to production with: .\deploy-platform.ps1 -Production"
}
